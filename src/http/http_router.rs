...
impl HttpRouter{
    ...
    pub fn handle(&mut self, mut http_request: HttpRequest){
        ...
        for part in path.split("/"){
            ...
            else if let Some(inner_node) = routes.iter().map(|x| x.1).find(|x| x.wildcard == true || x.var_name.is_some()){
...
mod tests {
    ...
    fn it_calls_route_with_right_value() {
        ...
        let on_handler = move |x: HttpRequest| *handler_called_copy.lock().unwrap() = String::from(x.route_params.get("key").unwrap().clone());
